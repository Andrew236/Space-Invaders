...
class Enemy(Ship):
    TYPE_MAP = {
                "alien": (ALIEN,BULLET),
                "guy": (ENEMY, BULLET)}
    def __init__(self,x,y,typee,health=100):
        super().__init__(x,y,health)
        self.ship_img, self.laser_img = self.TYPE_MAP[typee]
        self.mask = pygame.mask.from_surface(self.ship_img)
    ...
    while run:
        ...
        if len(enemies) == 0:
            ...
            for i in range(wave_length):
                enemy = Enemy(random.randrange(50, WIDTH-100), random.randrange(-1500,-100), random.choice(["alien", "guy"]))
